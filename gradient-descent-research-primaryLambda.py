...
def mininize(_a):
    ...
    for i in range(0, size_of_y):
        l_min = minimize_scalar(lambda l: norm(minimizedFunction(_a - l * gradient[i]))).x
        lambda_vector.append(l_min)
        nrm = norm(minimizedFunction(_a - l_min * gradient[i]))
        sumNorms += nrm
        norms.append(nrm)
    ...
    _result = _a
    for i in range(0, size_of_y):
        print("part of influence:", math.sqrt(norms[i]/sumNorms))
        _result = _result - math.sqrt(norms[i]/sumNorms)*lambda_vector[i] * gradient[i]
